...
import static dao.LoginDAO.uname;
import java.sql.*;
import java.util.*;
...
public class ProductManagementDAO {
	...
	public static List<Product> getAllProducts()
	{
		...
		try
		{
			Connection conn = DBUtil.getConnection();
                        PreparedStatement ps= conn.prepareStatement("SELECT * FROM product where username = ?");
	...
	public static Product getProductById(String productId)
	{
		...
		{
			Connection conn = DBUtil.getConnection();
			PreparedStatement ps= conn.prepareStatement("SELECT * FROM product WHERE prod_id = ? AND username = ?");
	...
	public static int updateProduct(Product product)
	{
		...
		{
			Connection conn = DBUtil.getConnection();
			PreparedStatement ps= conn.prepareStatement("UPDATE product SET prod_name=?, prod_category=?, prod_price=? WHERE prod_id=? AND username = ?" );
	...
	public static int deleteProduct(String productId)
	{
		...
		{
			Connection conn = DBUtil.getConnection();
			PreparedStatement ps= conn.prepareStatement("DELETE FROM product where prod_id = ? AND username =?");
			ps.setString(1, productId);
                        ps.setString(2,uname);
			status = ps.executeUpdate();
		}
